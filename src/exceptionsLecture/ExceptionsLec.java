...
//            general catch specific catch
//e is used in general to catch an exception
//Errors indcate smthng is v wrong
//checked(must be handled in code via compiler (our code won't compile) unchecked(runtime exception) and errors(os/jvm level)
//    if we don;'t handle exceptions in a try catch block must be put in function definition
//    be specific on flow of program use a throw catch
//    print stack trace will tell us what the error is